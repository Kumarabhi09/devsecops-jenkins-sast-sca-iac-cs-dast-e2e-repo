pipeline {
                ...
                withCredentials([string(credentialsId: 'SNYK_TOKEN', variable: 'SNYK_TOKEN')]) {
                    script {
                        try {
                            ...
                            echo "[WARNING] Snyk CLI scan failed: ${err.getMessage()}"
                        }
                    }
                }
            }
                ...
                withCredentials([string(credentialsId: 'SNYK_TOKEN', variable: 'SNYK_TOKEN')]) {
                    script {
                        try {
                            ...
                            echo "[WARNING] Snyk SCA Maven Plugin failed: ${err.getMessage()}"
                        }
                    }
                }
            }
            ...
            steps {
                script {
                    try {
                        ...
                        echo "[WARNING] ZAP DAST scan failed: ${err.getMessage()}"
                    }
                }
            }
        }
            ...
            steps {
                script {
                    try {
                        ...
                        echo "[WARNING] Checkov IaC scan failed: ${err.getMessage()}"
                    }
                }
            }
        }
    ...
    post {
        always {
            echo 'Pipeline finished. Check reports and logs for details.'
        }
        success {
